...
for i in range(0,10000,250):
    ...
    print zipPar
    pool=Pool(5)
    pool.map(multi_run_wrapper,zipPar) ##map执行组操作
    #print results
    pool.close()
    pool.join() #阻塞
    time.sleep(10)
...
#多线程形式二，map与apply（xxx） 两种用法 
import random 
import time
import itertools
import numpy as np
from multiprocessing import Pool
for i in range(250,100001,250):
    proxy=getip()
    ip=proxy[0]
    ip_sample=random.sample(ip,10)
    ip_sample=ip_sample*25
    pool=Pool(10)
    for j in range(i-250,i):
        k=0
        zipPar=zip([j],[j+1],[ip_sample[k]])
        print zipPar       
        pool.apply(multi_run_wrapper,zipPar)#map执行组操作
        #print results
        k=k+1        
        pool.close()
        pool.join() #阻塞
